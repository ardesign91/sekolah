	...
	if ( page <= parseInt(total_page) ) {
		...
		$.post( _BASE_URL + 'public/employee_directory/get_employees', data, function( response ) {
			...
			for (var z in rows) {
				...
				str += '<img src="' + row.photo + '" class="card-img img-fluid p-2">';
				str += '</div>';
			...
			<div class="row">
				...
				<div class="col-lg-12 col-md-12 col-sm-12 " data-aos="fade-up" data-aos-delay="200">
					...
					<div class="justify-content-between align-items-center float-right mb-3 w-100 more-employees">
						<button type="button" onclick="get_employees()" class="btn btn-info float-right"><i class="fa fa-refresh"></i> Tampilkan Lebih Banyak</button>
					</div>
				</div>
			</div>
		</div>
